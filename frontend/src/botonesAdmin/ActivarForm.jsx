...
function OnOffFormulario() {
  ...
  const editarEstadosIndividuales = async (id) => {
    try {
      await fetch(
        "http://127.0.0.1:5010/usuario/editarEstadosFormulariosIndividuales",
        {
          method: "PUT",
  ...
  const editarEstadosActivados = async (id) => {
    try {
      await fetch(
        "http://127.0.0.1:5010/usuario/editarEstadosFormulariosActivados",
        {
          method: "PUT",
          body: JSON.stringify({ id }),
          headers: {
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th>Activar</th>
              <th>
                <button onClick={editarEstados}>Desactivar Todos</button>
              </th>
            </tr>
          </thead>
          <tbody>
            {data.map((elementos) => (
              <tr key={elementos.id}>
                ...
                <td>
                  <button
                    className="Activarbutton"
                    onClick={() => editarEstadosActivados(elementos.id)}
                  >
                    Activar
                  </button>
                </td>
                <td>
                  <button
                    onClick={() => editarEstadosIndividuales(elementos.id)}
                  >
                    Desactivar
                  </button>
                </td>
